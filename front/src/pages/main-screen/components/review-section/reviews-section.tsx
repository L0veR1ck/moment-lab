...
function ReviewsSection() {
  return (
    <section className="flex flex-col w-full py-[64px] max-w-[1280px]">
      <h2 className="text-center font-semibold text-[64px] text-[var(--color-dark-blue)] pb-[56px]">
        Отзывы
      </h2>
      <Carousel
      ...
      >
        {reviews.map((item) => (
          <div
            key={item.id}
            className="border-b-[50px] border-b-[var(--color-beige)] px-4 h-full"
          >
            <div className="min-h-[250px] pt-[24px] px-[24px] bg-[var(--color-blue)]/15 rounded-lg shadow p-6 h-full flex flex-col gap-[24px] text-left">
              <span className="font-semibold text-lg text-[var(--color-blue)]">
                {item.author}
              </span>
